...
public class RegisterCheCiActivity extends BaseActivity implements CTitleBar.CTitleBarContainer {
    ...
    public void initializeTitleBar(CTitleBar titleBar) {
        titleBar.setTitle("车次功能号");
    }

    @Override
    protected int getContentViewLayoutID() {
        return R.layout.activity_register_che_ci;
    }
    ...
    @OnClick({R.id.btQuery, R.id.btRegister,R.id.tv_Gnm})
    public void onViewClicked(View view) {
        switch (view.getId()) {
            case R.id.btQuery: {
                ...
                LogInstance.debug(GlobalPara.Tky, "查询车次功能号:" + strZm + "、" + strSz + "、" + strGnm);//

                String strFn = strZm + strSz;
                int iStatus = Integer.parseInt(strGnm);
                strFn = GlobalFunc.TrainNumberToFunctionNumber(strFn, iStatus);
                ...
                if (ret == -1) {
                    ToastUtil.showLongToast("车次功能号查询失败");
                }
            }
                break;
            case R.id.btRegister: {
                ...
                String strSz = etCsz.getText().toString().trim();
                String strGnm = getGnm(tvGnm.getText().toString().trim());
                LogInstance.debug(GlobalPara.Tky, "注册车次功能号:" + strZm + "、" + strSz + "、" + strGnm);//
                ...
                int ret = mLteHandle.TrainNumberRegisterOrNot(strFn, iStatus, true);
                if (ret == -1) {
                    ToastUtil.showLongToast("车次功能号注册失败");
                } else {
                    finish();
                }
            }
